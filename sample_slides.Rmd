---
title: "Text Visualization Workshop"
author: "Josemari Feliciano"
institute: "MPH in Biostatistics Candidate"
subtitle: "Yale School of Public Health"
date: "2020/02/26"
output:
...
background-image: url(http://rstudio.jfeliciano.io/files/part2.png)
...
![](http://rstudio.jfeliciano.io/files/datasource.png)
...
- In R, we refer to this structure as a data frame. Suppose `document_df` is the name of the said data frame.

```{r echo=FALSE}
document_df <- data.frame(year=c(1999,2020), text = c("Hello World!", "Where did you go?"), stringsAsFactors = FALSE)
knitr::kable(head(document_df), format = 'html')
```
...
- Understanding how to access and manipulate data frame is an important skill for this workshop. It is fairly straight forward.
...
```{r eval=FALSE}
document_df$year <- document_df$year - 1999
```
...
```{r eval=FALSE}
document_df$num_char <- nchar(document_df$text)
```
...
Text processing often involves text matching that is case and sensitive.

To get around this, we can take advantage of `gsub()` which I argue is one of the best tools, though it is often underutilized.

```{r eval= FALSE}
document_df$text <- gsub("[[:punct:]]", "", document_df$text) 
```
...
To get around this, we can take advantage of tolower()

```{r eval= FALSE}
document_df$text <- tolower(document_df$text) 
```
